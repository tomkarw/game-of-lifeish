    ...
    def __init__(self,pixWidth,pixHeight,pixel):
        ...
        for i in range(nColonies):
            for n in range(1000):
                ...
                l = numpy.random.normal(life,1) + pow(2,(s-50)/5)      # life span altered by plus/minus normal distribution
                a = random.random()*4
    ...
    def actBlobs(self):
        ...
        for x in range(self.pixWidth):
                for y in range(self.pixHeight):
                    ...
                    for i in range(1,nColonies):
                        self.array[x][y][i].sort(key=lambda Blob: Blob.strength,reverse=True) 
